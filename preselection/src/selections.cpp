...
RNode METFilters(RNode df_) {
    return df_.Define("passesMETFilters", "Flag_goodVertices && "
...
RNode ElectronSelections(RNode df_) {
    return df_.Define("Electron_SC_eta", "Electron_eta + Electron_deltaEtaSC")
            .Define("looseElectrons", 
                ...
                "Electron_cutBased >= 2 && "
                "Electron_lostHits <= 1")
            .Define("nVetoElectrons", "nElectron == 0 ? 0 : Sum(looseElectrons)")
...
RNode MuonSelections(RNode df_) {
    return df_.Define("looseMuons", 
                ...
                "abs(Muon_sip3d) < 8 && "
                "Muon_looseId == 1")
            .Define("nVetoMuons", "nMuon == 0 ? 0 : Sum(looseMuons)")
...
RNode EventSelections(RNode df_) {
     return df_.Define("passCut1", "passesMETFilters && passesTriggers")
        .Define("passCut2", "passCut1 && ((nVetoMuons == 2 && nTightMuons == 2 && nVetoElectrons == 0 && nTightElectrons == 0) || "
            "(nVetoMuons == 0 && nTightMuons == 0 && nVetoElectrons == 2 && nTightElectrons == 2)) && "
            "(lepton_pt > 40)")
